...
class srcSEDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        h, w ,c  = opt_img.shape
        #opt_img = opt_img[128:, :]
        #opt_img = cv2.copyMakeBorder(opt_img, 128, 0, 0,0, cv2.BORDER_CONSTANT, value=(0,0,0))
        #h_ratio = h / 320
        #w_ratio = w / 320
        ...
        #sar_img = cv2.cvtColor(sar_img,cv2.COLOR_BGR2GRAY)
        ...
        #sar_img = cv2.resize(sar_img, (512, 512))
        #opt_img = cv2.resize(opt_img, (512, 512))
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(
        srcSEDataset("/home/ly/data/dataset/train.txt",size=size),
        batch_size=4,
        shuffle=True,
        num_workers=0,
        pin_memory=True)
...
def build_src_SE(
        ...
        size,
        stride):
    train_data = srcSEDataset(
        train_data_file,
        size=(320, 320),
        stride=8)
    test_data = srcSEDataset(
        test_data_file,
        size=(320, 320),
        stride=8)
